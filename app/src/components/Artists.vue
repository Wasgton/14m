  ...
  <section id="artists" class="py-32 bg-zinc-900/50">
    <div class="max-w-7xl mx-auto px-6">
      <div class="text-center mb-20">
        <h2 class="text-5xl md:text-6xl font-black uppercase tracking-tight mb-6">Nossos <span class="text-fuchsia-500">Artistas</span></h2>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
        <div v-for="artist in artists" :key="artist.id" class="group relative aspect-[3/4] rounded-[2rem] overflow-hidden cursor-pointer shadow-xl">
          ...
          <div class="absolute inset-0 border-2 border-transparent group-hover:border-fuchsia-500/30 rounded-[2rem] transition-colors duration-500 z-20 pointer-events-none"></div>
          ...
          <div class="absolute bottom-0 left-0 w-full p-8 text-center transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500 z-10">
            <span class="inline-block px-4 py-1.5 rounded-full bg-fuchsia-600/90 backdrop-blur-sm text-white font-bold text-xs uppercase tracking-widest mb-3">{{ artist.genre }}</span>
